...
from learnable_tokenizers.spixelFCN.train_util import *
...
superpixel_model = SpixelNet().to(device)

criterion = nn.CrossEntropyLoss()
optimizer = optim.Adam(list(stn.parameters()) + list(model.parameters()) + list(superpixel_model.parameters()), lr=0.001)
...
for epoch in range(10):
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        for image, segments in zip(data, segmentation_labels):
            segmented_features = []
