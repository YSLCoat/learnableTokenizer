...
def init_spixel_grid(args,  b_train=True):
    ...
    n_spixl_h = int(np.floor(img_height/16))
    n_spixl_w = int(np.floor(img_width/16))
    ...
    torch_spix_idx_tensor = torch.from_numpy(
                np.tile(spix_idx_tensor, (128, 1, 1, 1))).type(torch.float).cuda()
    ...
    all_XY_feat = (torch.from_numpy(
        np.tile(coord_tensor, (128, 1, 1, 1)).astype(np.float32)).cuda())
...
def spixlIdx(args, b_train = False):
    ...
    if b_train:
        n_spixl_h = int(np.floor(224 / 16))
        n_spixl_w = int(np.floor(224 / 16))
    else:
        n_spixl_h = int(np.floor(224 / 16))
        n_spixl_w = int(np.floor(224 / 16))
