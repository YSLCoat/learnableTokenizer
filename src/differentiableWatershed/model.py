...
class VoronoiPropagation(nn.Module):
    def __init__(self, num_clusters=196, height=224, width=224, n_input_channels=3, embed_dim=192, device='cpu'):
        """
        Args:
        ...
        self.cnn = nn.Sequential(
            nn.Conv2d(n_input_channels, 64, kernel_size=3, stride=1, padding=1),  # Standard convolution
            nn.BatchNorm2d(64),
            nn.ReLU(),
    ...
    def distance_weighted_propagation_single_channel(self, centroids, grad_map, color_map, feature_descriptor,
                                                    num_iters=50, gradient_weight=10.0, color_weight=10.0,
                                                    edge_exponent=4.0, feature_weight=10.0):
        B, _, H, W = grad_map.shape
        mask = torch.full((B, H, W), fill_value=-1, device=grad_map.device)
        dist_map = torch.full((B, H, W), float('inf'), device=grad_map.device)
        ...
        for _ in range(num_iters):
            for dy, dx in directions:
                ...
                color_diff = torch.abs(color_map - torch.roll(color_map, (dy, dx), (2,3))).sum(dim=1)
                feat_diff = (feature_descriptor - torch.roll(feature_descriptor, (dy, dx), (1,2))).abs()
    ...
    def soft_min_of_two(self, a, b):
        # a, b: [B,C,H,W]
        # softmin(a,b) = -log(exp(-a)+exp(-b))
        # We'll combine them into a stack along a new dimension and reduce:
        stacked = torch.stack([a, b], dim=0)  # [2,B,C,H,W]
        softmin_val = -torch.logsumexp(-stacked, dim=0)  # [B,C,H,W]
        return softmin_val
    ...
    def forward(self, x):
        ...
        feature_maps = self.cnn(x)  # [B, D, H, W]

        # Reduce to a single-channel descriptor (e.g., mean of all channels)
        with torch.no_grad():
            feature_descriptor = feature_maps.mean(dim=1)  # [B, H, W]

        # Now integrate feature_descriptor into propagation:
        mask = self.distance_weighted_propagation_single_channel(centroids, grad_map, x, feature_descriptor)
